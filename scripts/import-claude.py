#!/usr/bin/env python3
"""Import persistent knowledge from a Claude (claude.ai) data export into Open Brain.

Usage:
    python import-claude.py /path/to/conversations.jsonl
    python import-claude.py /path/to/conversations-000.jsonl conversations-001.jsonl
    python import-claude.py '/path/to/conversations*.jsonl'
    python import-claude.py /path/to/conversations.jsonl --dry-run
    python import-claude.py /path/to/conversations.jsonl --auto
"""
...
def parse_conversations(path: Path) -> list[dict]:
    """Parse a Claude export file. Supports JSONL (one conversation per line) and JSON arrays."""
    text = path.read_text(encoding="utf-8")

    # Try JSON array first (single array of conversations)
    try:
...
def build_transcript(conversation: dict) -> str | None:
    """Extract a readable transcript from a Claude conversation. Returns None if too short."""
    title = conversation.get("name", "Untitled")
    chat_messages = conversation.get("chat_messages", [])

    # Sort by created_at to ensure correct ordering
    chat_messages = sorted(chat_messages, key=lambda m: m.get("created_at", ""))
    ...
    for sender, text in messages:
        prefix = "User" if sender == "human" else "Assistant"
